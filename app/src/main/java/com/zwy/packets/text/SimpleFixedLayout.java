...
public class SimpleFixedLayout extends Layout{
    public static final int ENCODE_HEX=1; //encode_hex
    ...
    private static void sInit(TextPaint p)//数据格式初始化
    {
        ...
        if (!sInited)
        {
            ...
            sSelectedText.setColor(Color.WHITE); //数据初始化颜色
            sSelectedBg.setColor(Color.parseColor("#2196F3"));
            sSSelectedBg.setColor(Color.BLUE); //选择后的颜色
    ...
    private void init(TextPaint paint) // 初始化数据字体大小字体格式
    {
    ...
    public void setOne(int w,int h)
    {
        ...
        if (h!=-1)  // if h == -1 默认值  ？
            mOneHeight=h;
        mColumns=(int)(getWidth()/ mOneWidth); //列数
        if (mColumns!=0)
        {
            if (mEncode==ENCODE_HEX) {
                if (mFile!=null)
                    mLineCount=(int)mFile.length / mColumns + 1;//总行数
                else
    ...
    public void select(int index)                                                                         //选择
    {
    ...
    public void drawText(Canvas canvas, int firstLine, int lastLine) {                               //不懂
        if (firstLine!=0)
            firstLine--;
