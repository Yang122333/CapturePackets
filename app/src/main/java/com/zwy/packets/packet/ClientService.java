...
public class ClientService extends VpnService{
    ...
    public static String addr="10.0.10.0";   //本地局域网
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        new Thread()
        {
            ...
            public void run() {
                ...
                ByteBuffer packet = ByteBuffer.allocate(32767); // 2 ^ 15 - 1 ??缓存大小      ？？ 16位
                int length;
                int errorTime=0;
                    while (mRunning) {
    ...
    private void build()
    {
        if (mInterface==null) {
            ...
            builder.setMtu(1500);//即表示虚拟网络端口的最大传输单元，如果发送的包长度超过这个数字，则会被分包；
            builder.addAddress(addr, 32);//虚拟网络端口的IP地址
            //builder.addAddress("127.0.0.1", port);
            builder.addRoute("0.0.0.0", 0);//将所有的IP包都路由到虚拟端口上去；
